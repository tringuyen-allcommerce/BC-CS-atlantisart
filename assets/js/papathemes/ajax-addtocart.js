...
import modalFactory, { defaultModal, ModalEvents } from '../theme/global/modal';
import swal from '../theme/global/sweet-alert';
import { openCartPreview } from './utils';
...
export default function (context) {
    ...
    $('body').on('click', '[data-papathemes-cart-item-add]', (event) => {
        ...
        const $parent = $(event.target).closest('form, .card');
        const qty = $parent.find(`input[name=qty_${productId}]:visible`).val();
        if (qty && qty.length > 0) {
        ...
        utils.api.cart.itemAdd(formData, (err, response) => {
            ...
            if (context.add_to_cart_popup === 'cart') {
                ...
                const defModal = defaultModal();
                if (defModal) {
                    defModal.close();
                }
                $('body').trigger('ajax-addtocart-item-added', productId);
                return openCartPreview(response.data.cart_item.id);
            }
