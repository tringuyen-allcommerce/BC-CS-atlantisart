...
const SizeClasses = {
    small: 'modal--small',
    large: 'modal--large',
    purchaseOptions: 'modal--purchaseOptions', // papathemes-inhealth
    normal: '',
};
...
function getSizeFromModal($modal) {
    ...
    // papathemes-inhealth
    if ($modal.hasClass(SizeClasses.purchaseOptions)) {
...
function restrainContentHeight($content) {
    ...
    const viewportHeight = getViewportHeight(0.98); // papathemes-beautify: according max-height: 98%
    const maxHeight = viewportHeight - (contentHeight - bodyHeight) - parseInt($body.css('marginTop'), 10) - parseInt($body.css('marginBottom'), 10); // papathemes-kansha: minus margins
...
export class Modal {
    ...
    set size(size) {
        ...
        this.$modal
            ...
            .removeClass(SizeClasses.purchaseOptions) // papathemes-inhealth
